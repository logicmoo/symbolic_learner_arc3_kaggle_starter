...
- [ ] **Partial** Extend `object_registry.pl` into persistent identities across examples, encounters, and transitions while preserving Phase 1 provenance.  
  ...
  Consecutive live encounters now produce unresolved proposals automatically, and the runner exposes explicit friendly-registry accept/reject controls that preserve attributable accounts and append-only Prolog decisions. Similarity never self-authorizes. Remaining: carry registry state across examples.
...
- [ ] **Partial** Recognize recurring objects under supported translation, rotation, scale, reflection, recoloring, noise, and partial visibility.

  Existing partial evidence: correspondence proposals explicitly recognize declared translation, rotation, scale, reflection, recoloring, noise, and partial-visibility changes. Missing properties under declared partial visibility produce support rather than false contradiction, and incomplete/noisier encounters cannot replace the best complete stored form. `ResidualAnalyzer` absorbs declared transformations but preserves every unexplained property change as a deterministic provisional residual, which live capture, README inspection, action-tree replay, and Prolog persistence retain. Real degraded fixtures and domain-specific deformation remain open.
...
- [ ] **Partial** Analyze observed transitions and infer candidate object-level transformations.  
  Existing evidence: `TransitionAnalyzer`, `TransformationLearner`, and Prolog equivalents plus canonical Phase 2 adapters. The analyzer validates real before/after payloads and preserves action and provenance. The learner retains every persisted direct object change and now infers explicit absolute-target and relative-delta rivals for numeric changes, preserving their common evidence while assigning distinct assumptions and critiques. The rule inducer keeps both interpretations as mutual rivals, and unseen-case execution demonstrates their observably different predictions. Remaining: infer and evaluate richer domain-specific alternatives beyond numeric absolute/relative interpretations.
