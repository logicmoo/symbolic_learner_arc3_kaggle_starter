...
- [`README.md`](README.md) — top-level documentation index, layered resource discovery, installation, and runnable commands.
...
- [`SOW_PHASE_ARCHITECTURE.md`](SOW_PHASE_ARCHITECTURE.md) — large technical overview containing classes, modules, data contracts, architecture boundaries, and detailed architecture work.
- [`TODO.md`](TODO.md) — concrete active implementation work in practical execution order.
- [`SOW_DELIVERABLES.md`](SOW_DELIVERABLES.md) — phase-by-phase SOW checklist with completed, partial, and open status plus evidence links.
...
- deliverables tracks what must be demonstrated and checked off.
...
## Phase 1 debugger, providers, and evidence

- [`python/arc3_runner.py`](python/arc3_runner.py) — ARC3 lifecycle, legal actions, level handling, history, replay, state capture, exports, and symbolic command entry points.
...
- [`python/llm_json_patch.py`](python/llm_json_patch.py) — response wrapper that records the initial output, performs local or text-only repair, and keeps interactions in one run record.
...
- [`python/action_tree.py`](python/action_tree.py) — deterministic filesystem action tree, state metadata, image hashes, parent/child links, generated READMEs, and level-wide friendly identities.
- [`python/gpt_bridge.py`](python/gpt_bridge.py) — provider-selected prompt, shared multimodal analysis request, artifact generation, normalization, and Prolog artifact output.
- [`python/swipl_bridge.py`](python/swipl_bridge.py) — subprocess bridge into SWI-Prolog; normalized symbolic queries extend this bridge.
...
- [`prolog/game_object_learner_api.pl`](prolog/game_object_learner_api.pl) — connected orchestration from transition analysis through rule storage, prediction, and later grading.
...
- `README.md` — state navigation, active transcript, historical run links, and embedded latest artifacts.
...
- `object_registry.pl` — authoritative friendly identities for the level.
- `objects.pl` — current-state facts.
- `differences.pl` — parent/current symbolic delta.
- `similarities.pl` — object correspondences.
- `turtle_from_image.pl` — current-state reconstruction.
- `turtle_from_diff.pl` — parent-to-current transformation.
- `rules.pl` — candidate rules and supporting context.
