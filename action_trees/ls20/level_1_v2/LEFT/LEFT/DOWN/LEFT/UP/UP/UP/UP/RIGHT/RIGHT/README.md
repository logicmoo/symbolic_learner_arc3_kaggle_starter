...
previous_state(action11, action10).
...
bounding_box(bottom_center_gate, 29, 25, 5, 5).
bounding_box(gate_gray_header, 29, 25, 5, 2).
bounding_box(gate_burgundy_panel, 29, 27, 5, 3).
...
bounding_box(green_status_block, 13, 61, 9, 2).
bounding_box(bottom_status_track, 22, 61, 33, 2).
...
center(bottom_center_gate, 31, 27).
center(gate_gray_header, 31, 26).
center(gate_burgundy_panel, 31, 28).
...
center(green_status_block, 17, 62).
center(bottom_status_track, 38, 62).
...
adjacent(bottom_center_gate, fortress_left_wing).
...
separated_by_gap(bottom_center_gate, blue_black_player, 6).
...
above(bottom_center_gate, fortress_inner_courtyard).
...
aligned_with(bottom_center_gate, fortress_inner_courtyard, vertical).
between(bottom_center_gate, fortress_left_wing, fortress_right_wing).
...
state(bottom_center_gate, shifted_right_two_steps).
...
state(green_status_block, expanded_by_two_cells).
state(cyan_status_blocks, three_lit_blocks).
state(action11, gate_moved_right).
state(action11, player_stationary).
state(action11, status_progress_advanced).
...
turtle_program(bottom_center_gate,
    [penup, set_pos(29,25), setcolor(light_gray), pendown, fill_rect(5,2),
     penup, set_pos(29,27), setcolor(burgundy), pendown, fill_rect(5,3)]).
...
turtle_program(green_status_block,
    [penup, set_pos(13,61), setcolor(green), pendown, fill_rect(9,2)]).

turtle_program(bottom_status_track,
    [penup, set_pos(22,61), setcolor(dark_gray), pendown, fill_rect(33,2)]).
