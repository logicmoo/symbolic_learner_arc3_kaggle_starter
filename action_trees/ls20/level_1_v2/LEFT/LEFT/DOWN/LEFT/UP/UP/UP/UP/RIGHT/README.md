# `ls20` level `1` — LEFT / LEFT / DOWN / LEFT / UP / UP / UP / UP / RIGHT
...
% State-specific facts for this action-tree node.
state_id(action10).
incoming_action(action10, action('ACTION4', {})).
previous_state(action10, action9).
...
bounding_box(bottom_center_gate, 24, 25, 5, 5).
bounding_box(gate_gray_header, 24, 25, 5, 2).
bounding_box(gate_burgundy_panel, 24, 27, 5, 3).
...
bounding_box(green_status_block, 13, 61, 8, 2).
bounding_box(bottom_status_track, 21, 61, 34, 2).
...
center(bottom_center_gate, 26, 27).
center(gate_gray_header, 26, 26).
center(gate_burgundy_panel, 26, 28).
...
separated_by_gap(bottom_center_gate, blue_black_player, 1).
...
state(bottom_center_gate, shifted_right_one_step).
...
state(action10, gate_moved_right).
state(action10, player_stationary).
state(action10, status_progress_advanced).
...
turtle_program(bottom_center_gate,
    [penup, set_pos(24,25), setcolor(light_gray), pendown, fill_rect(5,2),
     penup, set_pos(24,27), setcolor(burgundy), pendown, fill_rect(5,3)]).
...
turtle_program(green_status_block,
    [penup, set_pos(13,61), setcolor(green), pendown, fill_rect(8,2)]).

turtle_program(bottom_status_track,
    [penup, set_pos(21,61), setcolor(dark_gray), pendown, fill_rect(34,2)]).
