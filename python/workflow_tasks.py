...
DEFAULT_TASK_PATH = ROOT / "config" / "workflow_tasks.json"
...
class Task:
    ...
    def impl(self, wanted:str|None)->Impl:
        if wanted:
            ...
            raise LlmConfigurationError(f"Task {self.id!r} has no implementation {wanted!r}")
...
def load_tasks(path:Path=DEFAULT_TASK_PATH)->tuple[Task,...]:
    rows=[]
    for r in read_obj(path).get("tasks",[]):
        imps=[]
        ...
        rows.append(Task(txt(r["id"]),txt(r.get("label")) or txt(r["id"]),txt(r.get("description")),dict(r.get("inputs") or {}),dict(r.get("outputs") or {}),tuple(imps)))
    return tuple(rows)

class TaskAwareWorkflowRouter(WorkflowAwareLlmProviderRouter):
    def __init__(self,config_path,*,workflow_path=None,task_path=None,datatype_path=None,**kw):
        self.task_path=Path(task_path or os.getenv("ARC3_WORKFLOW_TASK_CONFIG") or DEFAULT_TASK_PATH).resolve()
        self.datatype_path=Path(datatype_path or os.getenv("ARC3_WORKFLOW_DATATYPE_CONFIG") or DEFAULT_DATATYPE_PATH).resolve()
        self.datatypes=tuple(read_obj(self.datatype_path).get("types",[]))
        self.datatype_by_id={txt(x["id"]):x for x in self.datatypes}
        self.tasks=load_tasks(self.task_path); self.task_by_id={x.id:x for x in self.tasks}
        for t in self.tasks:
            for d in (*t.inputs.values(),*t.outputs.values()):
                if d not in self.datatype_by_id: raise LlmConfigurationError(f"Task {t.id!r} references unknown datatype {d!r}")
        real=Path(workflow_path or os.getenv("ARC3_LLM_WORKFLOW_CONFIG") or ROOT/"config"/"llm_workflows.json").resolve()
        raw=read_obj(real); cooked=copy.deepcopy(raw); synthetic=[]; self.task_step_by_transaction_id={}
        for wf in cooked.get("llm_workflows",[]):
            wid=txt(wf.get("id")) or "workflow"; out=[]
            for n,step in enumerate(wf.get("steps",[]),1):
                if not isinstance(step,dict) or not step.get("task"): out.append(step); continue
                tid=txt(step["task"])
                if tid not in self.task_by_id: raise LlmConfigurationError(f"Unknown task {tid!r}")
                task=self.task_by_id[tid]; task.impl(txt(step.get("implementation")) or None)
                badin=set((step.get("inputs") or {}))-set(task.inputs); badout=set((step.get("outputs") or {}))-set(task.outputs)
                if badin or badout: raise LlmConfigurationError(f"Unknown task ports inputs={sorted(badin)} outputs={sorted(badout)}")
                sid=f"__task__{slug(wid)}__{slug(txt(step.get('id')) or str(n))}"
                synthetic.append({"id":sid,"label":task.label,"kind":"runner_method","runner_method":"__typed_task__","requires_vision":False,"combine_safe":False})
                self.task_step_by_transaction_id[sid]=copy.deepcopy(step)
        ...
        fd,name=tempfile.mkstemp(prefix="arc3_tasks_",suffix=".json");os.close(fd);self._task_workflow_path=Path(name)
        self._task_workflow_path.write_text(json.dumps(cooked),encoding="utf-8")
        super().__init__(config_path,workflow_path=self._task_workflow_path,**kw)
        self.workflow_path=real
        for t in self.tasks:
            for i in t.implementations:
                if i.species=="llm" and i.transaction not in self.transaction_by_id: raise LlmConfigurationError(f"Unknown task transaction {i.transaction!r}")
    def __del__(self):
        try: super().__del__()
        finally:
            try:self._task_workflow_path.unlink(missing_ok=True)
...
def disk_directory(e,inp,par):
    d=Path(txt(par.get("directory")) or os.getenv("ARC3_WORKFLOW_IMAGE_DIR","") or input("Image directory: ")).expanduser().resolve()
...
def remote_url(e,inp,par):
    u=txt(par.get("url")) or os.getenv("ARC3_WORKFLOW_IMAGE_URL","") or input("Image URL: ");_,_,r=node_root(e);d=r/"source_url";d.mkdir(exist_ok=True);raw=d/"download"
...
def video_frames(e,inp,par):
    src=Path(txt(par.get("path")) or os.getenv("ARC3_WORKFLOW_VIDEO","") or input("Video path: ")).resolve();_,_,r=node_root(e);d=r/"video_frames";d.mkdir(exist_ok=True)
...
def display(e,inp,par):
    ...
    opened=[]
    if os.getenv("ARC3_WORKFLOW_SHOW_IMAGES","1") not in {"0","false"}:
        for p in ps:
...
def execute_task(e,tx):
    r=e.router;c=r.task_step_by_transaction_id[tx.transaction_id];t=r.task_by_id[txt(c["task"])];i=t.impl(txt(c.get("implementation")) or None);inp=resolve_inputs(e,t,c);par=dict(c.get("parameters") or {})
    print(f"Task {t.label}: {i.label} [{i.species}]")
    if i.species=="python":vals=HANDLERS[i.handler](e,inp,par)
        ...
        e._active_task_images=[p for v in inp.values() for p in paths(v) if p.exists()]
        try:
            target=replace(target,include_parent_image=False,include_current_image=False) if e._active_task_images else target
        ...
        finally:e._active_task_images=[]
        vals=artifact_outputs(e,t)
    store_outputs(e,t,c,vals)

def install_task_workflows():
    from multillm_runner import MultiLlmArc3Runner
    if getattr(MultiLlmArc3Runner,"_arc3_typed_tasks_installed",False):return
    def router(self):
        if not isinstance(self._llm_router,TaskAwareWorkflowRouter):self._llm_router=TaskAwareWorkflowRouter(prompts_path())
        return self._llm_router
    def reload(self,*,active_model_id=None):
        self._llm_router=TaskAwareWorkflowRouter(prompts_path());self._gpt_analyzer=None
        if active_model_id:
            try:self._llm_router.select_model(active_model_id)
            except Exception:pass
        return self._llm_router
    MultiLlmArc3Runner.llm_router=router;MultiLlmArc3Runner.reload_llm_router=reload;MultiLlmArc3Runner._arc3_typed_tasks_installed=True
    oldrun=LlmWorkflowEngine.run
    def run(e,w):seed(e);oldrun(e,w);save_slots(e)
    run._arc3_typed_tasks=True;LlmWorkflowEngine.run=run
    oldrm=LlmWorkflowEngine._run_runner_method
    def rm(e,tx):
        if isinstance(e.router,TaskAwareWorkflowRouter) and tx.transaction_id in e.router.task_step_by_transaction_id:return execute_task(e,tx)
        return oldrm(e,tx)
    LlmWorkflowEngine._run_runner_method=rm
    oldreq=LlmWorkflowEngine._request_content
    def req(e,*a,**k):
        c=oldreq(e,*a,**k);ps=getattr(e,"_active_task_images",[])
        if not ps:return c
