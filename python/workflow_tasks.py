...
class TaskAwareWorkflowRouter(WorkflowAwareLlmProviderRouter):
    def __init__(self,config_path,*,workflow_path=None,task_path=None,datatype_path=None,**kw):
        self.task_path=Path(task_path or os.getenv("WORLD_WORKBENCH_TASK_CONFIG") or os.getenv("ARC3_WORKFLOW_TASK_CONFIG") or DEFAULT_TASK_PATH).resolve()
        ...
        raw=read_obj(real); cooked=copy.deepcopy(raw); cooked["llm_workflows"]=expand_subworkflows(cooked.get("llm_workflows") or []); synthetic=[]; self.task_step_by_transaction_id={}
