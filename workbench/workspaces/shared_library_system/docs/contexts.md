...
Design resources live under `design/atomspaces/`. Shared AtomSpaces are inherited, while a workspace resource with the same `id` overrides the shared definition. A same-kind `implements` relationship defines a concrete specialization without introducing a separate variant kind or directory.
