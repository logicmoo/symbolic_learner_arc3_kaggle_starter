"""color_names.py — the full CSS named-color table, built once, used to give
first-pass blobs a readable `name_color` label (nearest named color to a hex).

Kept as data + a single nearest-match function so any hex (ARC flat palette,
cartoon, or CGI) resolves to a stable human color name.
"""
from __future__ import annotations

# CSS Level 4 extended color keywords (name -> #rrggbb).
CSS_HEX: dict[str, str] = {
...
# Built once at import: name -> (r, g, b)
CSS_RGB: dict[str, tuple[int, int, int]] = {n: _to_rgb(h) for n, h in CSS_HEX.items()}


def nearest_name(hexs: str) -> str:
    """Nearest CSS color name for a #rrggbb hex (squared-distance in RGB)."""
    try:
        r, g, b = _to_rgb(hexs)
    except (ValueError, IndexError):
        return "color"
    best, bd = "color", 1 << 30
    for name, (nr, ng, nb) in CSS_RGB.items():
