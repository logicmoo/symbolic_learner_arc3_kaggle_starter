...
4. For each semantic operation, the resolver gathers all inherited and workspace-local implementation children.
5. Policy filters out implementations that are unavailable, unhealthy, unauthorized, incompatible, or too costly.
6. The resolver selects an existing implementation or asks an implementation-producing model to create a new child implementation.
