...
export function HelpDocumentTabs({preferred,context,onOpenDocs}:{preferred?:string;context?:string;onOpenDocs?:(filter:string)=>void}){
 ...
 useEffect(()=>{let cancelled=false;setDocs({});setErrors({});for(const tab of docTabs){const request=tab.repositoryPath?readRepository(tab.repositoryPath).then(document=>document.content):readShared(tab.path!);void request.then(content=>{if(!cancelled)setDocs(current=>({...current,[tab.id]:content}))}).catch(reason=>{if(!cancelled)setErrors(current=>({...current,[tab.id]:String(reason)}))})}return()=>{cancelled=true}},[]);
