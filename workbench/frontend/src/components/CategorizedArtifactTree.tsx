...
function openTopicEditor(path: string) { const url = new URL(window.location.href); url.searchParams.set("view", "topics"); url.searchParams.set("resource", path); window.history.pushState({}, "", url); window.dispatchEvent(new PopStateEvent("popstate")); }
function CategoryHeader({ label, itemCount, special, firstClass = false, path }: { label: string; itemCount: number; special?: "all" | "uncategorized"; firstClass?: boolean; path?: string }) { return <div className={`operation-tree-row operation-category-row ${special ? `category-${special}` : firstClass ? "category-first-class" : "category-virtual"}`}><span className="operation-kind-badge">CATEGORY</span><span><b>{!special && path ? <a href={`?view=topics&resource=${encodeURIComponent(path)}`} title={firstClass ? "Open in the Topic editor" : "Create this topic in the Topic editor"} style={{ color: "inherit", textDecoration: "underline dotted", cursor: "pointer" }} onClick={event => { event.preventDefault(); event.stopPropagation(); openTopicEditor(path); }}>{label}</a> : label}</b><small>{special ? (special === "all" ? "All resources" : "No topic") : firstClass ? "First-class topic" : "Virtual topic"}</small></span><em>{itemCount} items</em></div>; }
function renderCategory(node: CategoryNode, categoryCommand: ArtifactTreeCommand | undefined, firstClassPaths: Set<string>): ReactNode {
  ...
  return <ArtifactTreeBranch key={`category:${node.path}`} branchCommand={categoryCommand} className="operation-tree-group artifact-category-branch" label={title(node.name)} searchValue={{ category: node.path }} header={<CategoryHeader label={title(node.name)} itemCount={count(node)} firstClass={firstClassPaths.has(node.path)} path={node.path} />}>
