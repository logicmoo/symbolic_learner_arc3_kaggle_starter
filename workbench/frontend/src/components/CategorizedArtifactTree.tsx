...
function CategoryHeader({ label, itemCount, special, firstClass = false, path }: { label: string; itemCount: number; special?: "all" | "uncategorized"; firstClass?: boolean; path?: string }) { return <div className={`operation-tree-row operation-category-row ${special ? `category-${special}` : firstClass ? "category-first-class" : "category-virtual"}`}><span className="operation-kind-badge">CATEGORY</span><span><b>{firstClass && path ? <a href={`?view=topics&resource=${encodeURIComponent(path)}`} title="Open in the Topic editor" style={{ color: "inherit", textDecoration: "underline dotted", cursor: "pointer" }} onClick={event => { event.preventDefault(); event.stopPropagation(); openTopicEditor(path); }}>{label}</a> : label}</b><small>{firstClass ? "First-class topic" : "Virtual category"}</small></span><em>{itemCount} items</em></div>; }
