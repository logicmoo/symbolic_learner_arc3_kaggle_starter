...
function ConfigForm({source,onChange,items}:{source:string;onChange:(v:string)=>void;items:CatalogItem[]}){
 const doc=useMemo<ModelDef|null>(()=>{try{const parsed=JSON.parse(source) as ModelResource;return parsed.kind==="model"?{...parsed,inherits:modelParent(parsed)}:null}catch{return null}},[source]);
...
export function LlmModelsEditor({workspaceId}:{workspaceId:string}){
 ...
 return <HierarchyResourceEditor workspaceId={workspaceId} categoryTree="models" eyebrow="MODEL CATALOG" title="Backends, models & profiles" description="Models now use the same hierarchy/tree/tab/split-view editor shell as Operations, Data, and Prompts while keeping the rich model configurator." headerActions={actions} error={error} onDismissError={()=>setError(null)} leftPane={leftPane} tabs={tabs} activeKey={activeKey} compareKey={compareKey} onActivate={setActiveKey} onClose={close} renderEditor={(key,secondary)=>{const doc=openDocs.find(item=>item.key===key);return doc?renderEditor(doc,secondary):null}} emptyEditor={<div className="studio-empty">Select a backend, model, or profile.</div>} className="llm-model-editor model-hierarchy-page" treeClassName={`model-tree-pane ${layout}`} workspaceClassName="model-editor-workspace" tabsClassName="model-document-tabs" panesClassName="model-editor-panes"/>;
}
