...
function ConfigForm({source,onChange,items}:{source:string;onChange:(v:string)=>void;items:CatalogItem[]}){
 const doc=useMemo<ModelDef|null>(()=>{try{const parsed=JSON.parse(source) as ModelResource;return parsed.kind!=="backend"&&parsed.kind!=="system"?{...parsed,inherits:modelParent(parsed)}:null}catch{return null}},[source]);
