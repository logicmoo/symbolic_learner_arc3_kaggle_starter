...
export function LlmModelsEditor({workspaceId,catalogMode="models"}:{workspaceId:string;catalogMode?:"backends"|"models"}){
 ...
 const backends=useMemo(()=>catalogMode==="backends"?(snapshot?.backends||[]):(snapshot?.backends||[]).filter(isLlmBackend),[snapshot,catalogMode]);const nodes=catalogMode==="backends"?[]:snapshot?.models||[];
 ...
 return <HierarchyResourceEditor workspaceId={workspaceId} categoryTree="models" eyebrow={catalogMode==="backends"?"EXECUTION PROVIDERS":"MODEL CATALOG"} title={catalogMode==="backends"?"Backends":"Models & presets"} description={catalogMode==="backends"?"Configure LLM providers, Python, SWI-Prolog, MeTTa, MCP, shell, and workbench execution providers.":"Models inherit model backends; reusable presets inherit models or other presets and override invocation defaults without containing prompts."} headerActions={actions} error={error} onDismissError={()=>setError(null)} leftPane={leftPane} tabs={tabs} activeKey={activeKey} compareKey={compareKey} onActivate={setActiveKey} onClose={close} renderEditor={(key,secondary)=>{const doc=openDocs.find(item=>item.key===key);return doc?renderEditor(doc,secondary):null}} emptyEditor={<div className="studio-empty">{catalogMode==="backends"?"Select a backend.":"Select a model or preset."}</div>} className="llm-model-editor model-hierarchy-page" treeClassName={`model-tree-pane ${layout}`} workspaceClassName="model-editor-workspace" tabsClassName="model-document-tabs" panesClassName="model-editor-panes"/>;
}
