...
export function PddlPlanImportPanel({ workflow, onImported }: { workflow: Workflow; onImported: (workflow: Workflow) => void }) {
  ...
  return <section className="pddl-plan-import-panel"><div><span>PDDL GROUNDED PLAN IMPORT</span><b>Map planner actions to abstract Operations</b><small>The conversion is a draft: it does not save or run the Workflow. Equal temporal start times remain parallel.</small></div><label><span>ACTION MAP · JSON</span><textarea value={actionMapSource} onChange={event => setActionMapSource(event.target.value)} placeholder={'{"move":"navigation.move"}'} /></label><button disabled={busy || !workflow.planProvenance?.sourcePlan?.trim()} onClick={() => void convert()}>{busy ? "Converting…" : "Convert grounded plan to steps"}</button>{message && <p>{message}</p>}</section>;
}
