...
export function OperationPlayground({workspaceId,operation,variants,models=[],selectedImplementationVariant,onImplementationVariantChange,inputValues,onInvocationComplete,collapsed=false,onCollapsedChange}:{workspaceId:string;operation:OperationDef;variants:OperationImplementationDef[];models?:RunnerModel[];selectedImplementationVariant?:string;onImplementationVariantChange?:(implementationVariant:string)=>void;inputValues?:Record<string,unknown>;onInvocationComplete?:(outputs:Record<string,unknown>)=>void;collapsed?:boolean;onCollapsedChange?:(collapsed:boolean)=>void}){
 ...
 const preferred=runnableVariants.some(item=>item.id===selectedImplementationVariant)?selectedImplementationVariant!:runnableVariants.some(item=>item.id===operation.preferredChild)?operation.preferredChild!:runnableVariants[0]?.id||"";
 ...
 if(collapsed)return <section className="operation-playground operation-playground-collapsed"><button type="button" className="operation-playground-summary" onClick={()=>onCollapsedChange?.(false)}><span>COMPLETED</span><b>{operation.label||operation.id}</b><small>{result?`${result.implementation.label} · ${result.elapsedMs} ms`:"Debug details collapsed"}</small><em>Expand</em></button></section>;
 return <section className="operation-playground">
  <UniversalExecutionControls title="OPERATION PLAYGROUND" description="Inspect or execute this durable delayed Codex/agent task specification" actions={<><button type="button" title="Run the operation's saved default implementation" onClick={()=>run()} disabled={running}>{running?"Running…":"Run Default"}</button><button className="primary" type="button" title="Run the one-off implementation selected below" onClick={()=>run(invocationVariant)} disabled={running||!invocationVariant}>{running?"Running…":"▶ Run Selected"}</button></>} populating={populating} inputCount={inputs.length} onPopulate={populateInputs}/>
  <div className="operation-task-contract"><span>DELAYED TASK SPECIFICATION</span><b>{running?"running":result?"completed":error?"failed":"defined · not run"}</b><small>{operation.description||"Inspectable before execution; evidence can support agent/LLM routes, smaller models, ILP code synthesis, and resulting deterministic implementations."}</small></div>
   <div className="operation-playground-grid">
   <div className="operation-run-route"><label className="operation-playground-field"><span>RUN WITH {onImplementationVariantChange?"· WORKFLOW CASCADE":"(THIS RUN ONLY)"}</span><select value={invocationVariant} disabled={runnableVariants.length===1} onChange={event=>{setVariant(event.target.value);onImplementationVariantChange?.(event.target.value);clearInvocationResult()}}>{runnableVariants.map(item=><option key={item.id} value={item.id}>{item.label||item.id} · {item.implementation}</option>)}</select><small>{onImplementationVariantChange?"This implementation is used for individual tests and the next whole-workflow cascade.":selected?.id===fallback.id?"Uses the automatic openrouter/free LLM fallback for this run only. The saved default implementation is unchanged.":selected?`Executes ${selected.implementation} for this run. The saved default implementation is unchanged.`:"Select how this invocation should run."}</small></label><div className="operation-run-actions"><button type="button" title="Run the operation's saved default implementation" onClick={()=>run()} disabled={running}>{running?"Running…":"Run Default"}</button><button className="primary" type="button" title="Run the one-off implementation selected at left" onClick={()=>run(invocationVariant)} disabled={running||!invocationVariant}>{running?"Running…":"▶ Run Selected"}</button></div></div>
