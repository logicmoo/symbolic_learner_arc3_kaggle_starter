...
export function OperationPlayground({workspaceId,operation,variants,models=[],selectedImplementationVariant,onImplementationVariantChange,onDefaultImplementationChange,inputValues,onInvocationComplete,collapsed=false,onCollapsedChange}:{workspaceId:string;operation:OperationDef;variants:OperationImplementationDef[];models?:RunnerModel[];selectedImplementationVariant?:string;onImplementationVariantChange?:(implementationVariant:string)=>void;onDefaultImplementationChange?:(implementationVariant:string)=>void;inputValues?:Record<string,unknown>;onInvocationComplete?:(outputs:Record<string,unknown>)=>void;collapsed?:boolean;onCollapsedChange?:(collapsed:boolean)=>void}){
 ...
 return <section className="operation-playground">
   ...
   <UniversalExecutionControls actions={null} populating={populating} inputCount={inputs.length} onPopulate={mode=>populateInputs(mode)}/>
