...
type Props = { value: string; onChange: (json: string) => void; className?: string; style?: CSSProperties; label?: string };

export function ResourceSourceEditor({ value, onChange, className = "", style, label = "Edit this resource directly" }: Props) {
  ...
  return <div className="operation-json-block resource-source-editor">
    <div className="llm-subhead"><div><span>RESOURCE SOURCE</span><b>{label}</b></div><div className="source-format-tabs"><button className={format === "metta" ? "active" : ""} onClick={() => setFormat("metta")}>MeTTa</button><button className={format === "json" ? "active" : ""} onClick={() => setFormat("json")}>JSON</button></div></div>
