...
export function PromptLibraryEditor({workspaceId}:{workspaceId:string}){
 ...
 const newPrompt=()=>{const id="new_prompt";const doc:PromptDef={kind:"prompt",id,label:"New Prompt",description:"Abstract prompt contract.",inputs:{text:"text"},outputs:{text:"text"},implementationSelection:{default:`${id}.default`,variants:[`${id}.default`]}};open({path:`prompts/${id}.prompt.json`,source:workspaceId==="shared"?"shared":"workspace",workspaceId,document:doc})};
 const newImplementation=(parent:PromptDef)=>{const id=`${parent.id}.alternative`;const doc:PromptImplementationDef={kind:"prompt_implementation",id,implements:parent.id,label:`${parent.label||parent.id} — Alternative`,version:1,targets:["generic-chat"],text:["Implement the abstract prompt contract.","Return only the requested output."]};open({path:`prompts/${slug(id)}.prompt_implementation.json`,source:workspaceId==="shared"?"shared":"workspace",workspaceId,document:doc})};
 const saveDoc=(doc:OpenDocument)=>perform(async()=>{let document:PromptResource;try{document=JSON.parse(doc.source) as PromptResource}catch{throw new Error("Prompt resource JSON is invalid")};if(!document.id||!["prompt","prompt_implementation"].includes(document.kind))throw new Error("Prompt resource requires id and a valid kind");if(document.kind==="prompt_implementation"&&!document.implements)throw new Error("Prompt implementation requires implements");const original=doc.record.path;const path=workspaceId==="shared"||doc.record.source==="workspace"?original:`prompts/${slug(document.id)}.${document.kind}.json`;await request(`/api/workspaces/${encodeURIComponent(workspaceId)}/file`,{method:"PUT",body:JSON.stringify({path,content:JSON.stringify(document,null,2)})});const next=await load();const h=next.promptLibrary?.hierarchy;const pool:RecordFile<PromptResource>[]=[...(h?.prompts||next.prompts||[]),...(h?.promptImplementations||[])];const saved=pool.find(row=>row.document?.id===document.id);if(saved){const newKey=recordKey(saved);setOpenDocs(current=>current.map(item=>item.key===doc.key?{key:newKey,record:saved,source:JSON.stringify(saved.document,null,2),dirty:false}:item));if(activeKey===doc.key)setActiveKey(newKey);if(compareKey===doc.key)setCompareKey(newKey)}});
