...
export function PromptLibraryEditor({workspaceId}:{workspaceId:string}){
 ...
 const featuredPrompts=useMemo(()=>[...prompts].sort((a,b)=>{
   const aid=a.document?.id||"",bid=b.document?.id||"";
   const ac=(children[aid]||[]).length,bc=(children[bid]||[]).length;
   if(aid==="titlecase_received_text"&&bid!=="titlecase_received_text")return -1;
   if(bid==="titlecase_received_text"&&aid!=="titlecase_received_text")return 1;
 ...
 useEffect(()=>{if(payload&&!selected&&featuredPrompts[0])open(featuredPrompts[0] as RecordFile<PromptResource>)},[payload,featuredPrompts,selected]);
 ...
 return <section className="resource-view task-hierarchy-page">
  ...
  <div className="task-hierarchy-layout">
   <div className="task-tree-pane">
    {featuredPrompts.map(prompt=>{const item=prompt.document;if(!item)return null;const variants=children[item.id]||[];const declared=item.implementationSelection?.variants||[];return <div className="task-tree-group" key={item.id}>
  ...
  <div className="demo-notice"><b>Hierarchy proof</b><span>{prompts.length} abstract prompt(s) · {implementations.length} concrete alternative(s). The prompt with the most alternatives is placed first, with the bundled Titlecase prompt explicitly preferred.</span></div>
 </section>;
}
