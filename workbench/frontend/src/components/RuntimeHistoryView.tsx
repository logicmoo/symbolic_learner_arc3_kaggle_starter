...
type RuntimeRun = {
  ...
  artifacts: Array<{ id: string; stepId?: string; name: string; datatype?: string; representation?: string; payload?: unknown; contentHash?: string; provenance?: Record<string, unknown>; createdAt?: string; redacted?: boolean }>;
...
function objectArtifactRecords(artifact: RuntimeRun["artifacts"][number]): Record<string, unknown>[] {
  const payload = artifact.payload;
  if (Array.isArray(payload)) return payload.filter(item => item && typeof item === "object") as Record<string, unknown>[];
  if (!payload || typeof payload !== "object") return [];
  const record = payload as Record<string, unknown>;
  for (const key of ["objects", "entities", "objectAnnotations"]) {
    if (Array.isArray(record[key])) return (record[key] as unknown[]).filter(item => item && typeof item === "object") as Record<string, unknown>[];
  }
...
function WorkflowRunProjection({ run, workflow, busy, onCommand, onOpenResource, commands = ["pause", "resume", "advance", "replay", "cancel"] }: { run: RuntimeRun; workflow: FrozenWorkflow | null; busy: boolean; onCommand: (command: WorkflowRunCommand) => void; onOpenResource?: OpenRuntimeResource; commands?: WorkflowRunCommand[] }) {
  ...
  const chronologyPositions = run.events.map((_event, index) => ({ x: 65 + index * 135, y: 82 + (index % 3) * 62 }));
  ...
  const objectRecords = run.artifacts.flatMap(artifact => objectArtifactRecords(artifact).map(record => ({ artifact, record })));
  ...
  return <section className="run-projection" aria-label="Selected workflow run projection">
    ...
    {view === "topology" ? workflow ? <div className="run-topology-scroll">
      ...
      : <div className="run-chronology-scroll" aria-label="Durable event chronology">{run.events.length ? <svg className="run-chronology" viewBox={`0 0 ${chronologyWidth} 280`} style={{ minWidth: chronologyWidth }}><defs><marker id={`chronology-arrow-${run.id}`} viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto"><path d="M0 0L10 5L0 10z" /></marker></defs>{run.events.slice(1).map((event, index) => { const from = chronologyPositions[index], to = chronologyPositions[index + 1], midpoint = (from.x + to.x) / 2; return <path key={`event-edge:${event.id}`} className="run-chronology-edge" d={`M${from.x + 22},${from.y} C${midpoint},${from.y} ${midpoint},${to.y} ${to.x - 22},${to.y}`} markerEnd={`url(#chronology-arrow-${run.id})`} />; })}{run.events.map((event, index) => { const point = chronologyPositions[index], selected = String(event.id) === selectedEventId, state = /fail|error|cancel/i.test(event.kind) ? "failed" : /wait|human/i.test(event.kind) ? "waiting" : /start|running|resume/i.test(event.kind) ? "running" : "completed"; return <g key={event.id} transform={`translate(${point.x},${point.y})`} className={`run-chronology-node ${state} ${selected ? "selected" : ""}`} role="button" tabIndex={0} aria-label={`${index + 1}. ${event.kind}, ${event.stepId || "workflow"}, ${stamp(event.createdAt)}`} onClick={() => selectEvent(event)} onKeyDown={keyboardEvent => { if (keyboardEvent.key === "Enter" || keyboardEvent.key === " ") selectEvent(event); }}><circle r="22" /><text className="event-index" textAnchor="middle" y="3">{index + 1}</text><text className="event-kind" textAnchor="middle" y="38">{event.kind.length > 20 ? `${event.kind.slice(0, 18)}…` : event.kind}</text><text className="event-step" textAnchor="middle" y="50">{event.stepId || "workflow"}</text></g>; })}</svg> : <div className="studio-empty">This run has no durable events.</div>}</div>}
    ...
    {objectRecords.length > 0 && <div className="run-detected-objects"><div className="run-visual-comparison-title"><span>DETECTED OBJECTS</span><small>{objectRecords.length} persisted object records</small></div>{objectRecords.map(({ artifact, record }, index) => { const representationId = String(record.representation || artifact.representation || ""); const operationId = String(record.operationId || artifact.provenance?.operationId || ""); const modelId = String(record.modelId || artifact.provenance?.modelId || ""); return <article key={`${artifact.id}:${String(record.id || index)}`}><span>{index + 1}</span><div><b>{String(record.label || record.name || record.id || `Object ${index + 1}`)}</b><small>{String(record.type || record.kind || artifact.datatype || "Object")}{typeof record.confidence === "number" ? ` · confidence ${record.confidence.toFixed(2)}` : ""}</small></div>{onOpenResource && <div className="runtime-resource-links">{representationId && <button type="button" onClick={() => onOpenResource("datatype", representationId)}>Representation · {representationId}</button>}{operationId && <button type="button" onClick={() => onOpenResource("operation", operationId)}>Operation · {operationId}</button>}{modelId && <button type="button" onClick={() => onOpenResource("model", modelId)}>Model · {modelId}</button>}</div>}</article>; })}</div>}
