...
export function TaskLibraryEditor({workspaceId}:{workspaceId:string}){
 ...
 useEffect(()=>{if(snapshot&&openDocs.length===0){const first=snapshot.tasks?.[0] as RecordFile<TaskResource>|undefined;if(first)open(first)}},[snapshot]);
 ...
 const renderEditor=(doc:OpenDocument,secondary=false)=>{let document:TaskResource|null=null;try{document=doc.source?JSON.parse(doc.source) as TaskResource:null}catch{document=null}const selectedImplementation=document?.kind==="task_implementation"?document:null;const selectedModels=selectedImplementation?.modelSelection?.models||[];const selectedPrompts=Array.isArray(selectedImplementation?.bindings?.prompts)?selectedImplementation!.bindings!.prompts as string[]:[];
