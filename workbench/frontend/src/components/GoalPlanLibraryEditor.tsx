...
type Specification = { kind: BaseKind; id: string; implements?: Record<string, unknown>; label?: string; description?: string; enabled?: boolean; specializations?: Record<string, unknown>; preferredSpecialization?: string; goals?: string[]; successCriteria?: string[]; bindings?: string[]; [key: string]: unknown };
...
type Hierarchy = { specifications: RecordFile<Specification>[]; variants: RecordFile<Variant>[]; specializationsByResource: Record<string, RecordFile<Variant>[]> };
...
export function GoalPlanLibraryEditor({ workspaceId, family }: { workspaceId: string; family: Family }) {
  ...
  const specializations = payload?.hierarchy.specializationsByResource || {};
  ...
  const newSpecification = () => {
    ...
    const document: Specification = { kind: parentKind, id, label: `New ${familyLabel}`, description: family === "plan" ? "Strategy for selecting, generating, or adapting an executable workflow (a PDDL plan in PDDL terminology)." : `Abstract ${familyNoun} specification.`, specializations: {}, ...(family === "goal" ? { successCriteria: [] } : family === "plan" ? { goals: [] } : { bindings: [] }) };
  ...
  const newVariant = (parent: Specification) => {
    ...
    const document: Variant = { kind: parentKind, id, implements: implementsResource(parent.id), label: `${parent.label || parent.id} — Alternative`, description: `Concrete ${familyNoun} specialization.` };
  ...
  const renderEditor = (doc: OpenDocument, secondary = false) => {
    ...
    const alternatives = parent ? specializations[parent.id] || [] : [];
    const patchParent = (patch: Partial<Specification>) => parent && updateSource(doc.key, JSON.stringify({ ...parent, ...patch }, null, 2));
    const setPreferred = (id: string) => { if (!parent) return; const existing = specializationInheritanceMap(parent.specializations); const declaredSpecializations = Object.assign({}, ...alternatives.flatMap(row => row.document?.id ? [existing[row.document.id] ? { [row.document.id]: existing[row.document.id] } : specializesResource(row.document.id)] : [])); patchParent({ preferredSpecialization: id || undefined, specializations: declaredSpecializations }); };
    return <section className={`operation-editor-document ${secondary ? "secondary" : "primary"}`} key={doc.key}><div className="operation-editor-toolbar"><div><span>{parent ? `ABSTRACT ${familyLabel.toUpperCase()}` : `${familyLabel.toUpperCase()} ALTERNATIVE`}{doc.dirty ? " · UNSAVED" : ""}</span><h2>{document?.label || document?.id || doc.record.path}</h2><small>{doc.record.source} · {doc.record.path}</small></div><div className="operation-editor-actions">{!secondary && <button onClick={chooseComparison}>{compareKey ? "Single pane" : "Split view"}</button>}{parent && <button onClick={() => newVariant(parent)}>+ Alternative</button>}<button className="primary" disabled={busy || !document} onClick={() => saveDoc(doc)}>Save</button></div></div><div className="operation-editor-scroll">{!document && <div className="demo-notice"><b>Invalid resource</b><span>Fix the source before saving.</span></div>}{parent && <><div className="operation-abstract-summary"><div><span>PREFERRED ALTERNATIVE</span><select value={parent.preferredSpecialization || ""} onChange={event => setPreferred(event.target.value)}><option value="">planner-selected</option>{alternatives.map(row => <option key={row.document?.id} value={row.document?.id}>{row.document?.label || row.document?.id}</option>)}</select></div><div><span>SPECIALIZATIONS</span><code>{alternatives.length}</code></div><div><span>{family === "goal" ? "SUCCESS CRITERIA" : family === "plan" ? "GOALS" : "BINDINGS"}</span><code>{(family === "goal" ? parent.successCriteria : family === "plan" ? parent.goals : parent.bindings)?.length || 0}</code></div></div><div className="operation-model-list compact">{alternatives.map(row => { const specialization = row.document!; const preferred = parent.preferredSpecialization === specialization.id; return <button className={`operation-model-option ${preferred ? "selected" : ""}`} key={specialization.id} data-tree-search={JSON.stringify(specialization)} onClick={() => open(row as RecordFile<Resource>)}><span><b>{specialization.label || specialization.id}</b><small>{specialization.description || specialization.id}</small></span><em>{preferred ? "preferred" : "alternative"}</em></button>; })}</div></>}{variant && <div className="implementation-summary"><div><span>IMPLEMENTS</span><b>{relationshipIds(variant.implements).join(", ")}</b></div><div><span>WORKFLOW</span><b>{variant.workflow || "planner-selected"}</b></div></div>}{document&&<ResourceExecutionPlayground workspaceId={workspaceId} resource={document}/>}<ResourceSourceEditor value={doc.source} onChange={value => updateSource(doc.key, value)} label={`Edit this ${family} resource directly`} /></div></section>;
  };

  if (!payload) return <section className="resource-view"><div className="studio-empty">Loading {directory}…</div></section>;
  const leftPane = <>{ordered.map(record => { const item = record.document; if (!item) return null; const alternatives = specializations[item.id] || [], parentEnablement = resolveResourceEnablement(item); return <ArtifactTreeBranch key={item.id} label={item.label || item.id} searchValue={item} header={<div className="inheritance-row"><button className={`operation-tree-row operation-parent ${enablementClass(parentEnablement)} ${active?.record.document?.id === item.id ? "selected" : ""}`} onClick={() => open(record as RecordFile<Resource>)}><span className="operation-kind-badge">{familyLabel.toUpperCase()}</span><span><b>{item.label || item.id}</b><small>{item.description || item.id}</small></span><em>{alternatives.length} specializations <ResourceEnablementBadge state={parentEnablement} /></em></button><button className="hier-mini" onClick={() => newVariant(item)}>+ specialization</button></div>}>{alternatives.length ? alternatives.map(row => { const specialization = row.document!, specializationEnablement = resolveResourceEnablement(specialization, parentEnablement); const preferred = item.preferredSpecialization === specialization.id; return <button className={`operation-tree-row operation-child ${enablementClass(specializationEnablement)} ${active?.record.document?.id === specialization.id ? "selected" : ""}`} key={specialization.id} data-tree-search={JSON.stringify(specialization)} onClick={() => open(row as RecordFile<Resource>)}><span className="operation-kind-badge llm">SPEC</span><span><b>{specialization.label || specialization.id}</b><small>{specialization.description || specialization.id}</small></span><em>{preferred ? "preferred " : ""}<ResourceEnablementBadge state={specializationEnablement} /></em></button>; }) : undefined}</ArtifactTreeBranch>; })}</>;
