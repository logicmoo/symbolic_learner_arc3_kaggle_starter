...
type Selection = { default?: string; variants?: string[] };
type Specification = { kind: Family; id: string; label?: string; description?: string; variantSelection?: Selection; goals?: string[]; successCriteria?: string[]; [key: string]: unknown };
type Variant = { kind: "goal_interpretation" | "goal_variant" | "plan_variant"; id: string; implements: string; label?: string; description?: string; workflow?: string; [key: string]: unknown };
...
export function GoalPlanLibraryEditor({ workspaceId, family }: { workspaceId: string; family: Family }) {
  ...
  const newSpecification = () => {
    ...
    const document: Specification = { kind: family, id, label: `New ${familyLabel}`, description: `Abstract ${family} specification.`, variantSelection: { variants: [] }, ...(family === "goal" ? { successCriteria: [] } : { goals: [] }) };
  ...
  const newVariant = (parent: Specification) => {
    ...
    const document: Variant = { kind: variantKind, id, implements: parent.id, label: `${parent.label || parent.id} — Alternative`, description: `Concrete ${family} alternative.` };
  ...
  const saveDoc = (doc: OpenDocument) => perform(async () => {
    ...
    if (document.kind !== family && !(document as Variant).implements) throw new Error(`${familyLabel} variant requires implements`);
  ...
  const renderEditor = (doc: OpenDocument, secondary = false) => {
    ...
    const setPreferred = (id: string) => { if (!parent) return; const selection = parent.variantSelection || {}; const ids = alternatives.map(row => row.document?.id).filter(Boolean) as string[]; patchParent({ variantSelection: { ...selection, default: id || undefined, variants: ids } }); };
    return <section className={`task-editor-document ${secondary ? "secondary" : "primary"}`} key={doc.key}><div className="task-editor-toolbar"><div><span>{parent ? `ABSTRACT ${familyLabel.toUpperCase()}` : `${familyLabel.toUpperCase()} VARIANT`}{doc.dirty ? " · UNSAVED" : ""}</span><h2>{document?.label || document?.id || doc.record.path}</h2><small>{doc.record.source} · {doc.record.path}</small></div><div className="task-editor-actions">{!secondary && <button onClick={chooseComparison}>{compareKey ? "Single pane" : "Split view"}</button>}{parent && <button onClick={() => newVariant(parent)}>+ Alternative</button>}<button className="primary" disabled={busy || !document} onClick={() => saveDoc(doc)}>Save</button></div></div><div className="task-editor-scroll">{!document && <div className="demo-notice"><b>Invalid JSON</b><span>Fix the JSON before saving.</span></div>}{parent && <><div className="task-abstract-summary"><div><span>PREFERRED VARIANT</span><select value={parent.variantSelection?.default || ""} onChange={event => setPreferred(event.target.value)}><option value="">planner-selected</option>{alternatives.map(row => <option key={row.document?.id} value={row.document?.id}>{row.document?.label || row.document?.id}</option>)}</select></div><div><span>ALTERNATIVES</span><code>{alternatives.length}</code></div><div><span>{family === "goal" ? "SUCCESS CRITERIA" : "GOALS"}</span><code>{(family === "goal" ? parent.successCriteria : parent.goals)?.length || 0}</code></div></div><div className="task-model-list compact">{alternatives.map(row => { const child = row.document!; const preferred = parent.variantSelection?.default === child.id; return <button className={`task-model-option ${preferred ? "selected" : ""}`} key={child.id} onClick={() => open(row as RecordFile<Resource>)}><span><b>{child.label || child.id}</b><small>{child.description || child.id}</small></span><em>{preferred ? "preferred" : "alternative"}</em></button>; })}</div></>}{variant && <div className="implementation-summary"><div><span>IMPLEMENTS</span><b>{variant.implements}</b></div><div><span>WORKFLOW</span><b>{variant.workflow || "planner-selected"}</b></div></div>}<div className="task-json-block"><div className="llm-subhead"><div><span>RESOURCE JSON</span><b>Edit this {family} resource directly</b></div></div><textarea className="raw-json-editor task-visible-editor" value={doc.source} onChange={event => updateSource(doc.key, event.target.value)} /></div></div></section>;
  };

  if (!payload) return <section className="resource-view"><div className="studio-empty">Loading {directory}…</div></section>;
  const leftPane = <>{ordered.map(record => { const item = record.document; if (!item) return null; const alternatives = children[item.id] || []; return <div className="task-tree-group" key={item.id}><div className="inheritance-row"><button className={`task-tree-row task-parent ${active?.record.document?.id === item.id ? "selected" : ""}`} onClick={() => open(record as RecordFile<Resource>)}><span className="task-kind-badge">{familyLabel.toUpperCase()}</span><span><b>{item.label || item.id}</b><small>{item.description || item.id}</small></span><em>{alternatives.length} alternatives</em></button><button className="hier-mini" onClick={() => newVariant(item)}>+ alt</button></div><div className="task-tree-children">{alternatives.map(row => { const child = row.document!; const preferred = item.variantSelection?.default === child.id; return <button className={`task-tree-row task-child ${active?.record.document?.id === child.id ? "selected" : ""}`} key={child.id} onClick={() => open(row as RecordFile<Resource>)}><span className="task-kind-badge llm">ALT</span><span><b>{child.label || child.id}</b><small>{child.description || child.id}</small></span><em>{preferred ? "preferred" : ""}</em></button>; })}</div></div>; })}</>;
