...
type Workspace = {
  ...
  workflowDirectory:string; promptDirectory:string; configDirectory:string;
...
export function WorkspaceWorkbenchPage(){
  ...
  const chooseWorkspace=(item:Workspace)=>perform(async()=>{
    ...
    setWorkspace(item); setSnapshot(s); setImplementations(i.implementations); setCapabilities(c.capabilities);
  ...
  const saveFile=()=>perform(async()=>{
    if(!workspace)throw new Error('Select a workspace');
    let path=selectedPath;
    if(!path){
      const id=parsed?.id || 'workflow';
      path=`${workspace.workflowDirectory.replace(workspace.root.replace(/\\/g,'/'),'').replace(/^\//,'')}/${id}.json`;
    }
    await request(`/api/workspaces/${encodeURIComponent(workspace.id)}/file`,{method:'PUT',body:JSON.stringify({path,content:document})});
  ...
  const saveVersion=()=>perform(async()=>{
    ...
    setDocument(JSON.stringify(p.workflow,null,2));setValidation([]);
    await saveFile();
  });
  ...
  const start=()=>perform(async()=>{
    if(!parsed)throw new Error('Current file is not valid workflow JSON');
    let saved=parsed;
    if(!parsed.version){const p=await engine('/workflows',{method:'POST',body:JSON.stringify(parsed)});saved=p.workflow;setDocument(JSON.stringify(saved,null,2));}
